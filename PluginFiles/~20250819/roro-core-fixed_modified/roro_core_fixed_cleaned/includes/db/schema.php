...
--
--  These tables are imported from auxiliary SQL files (initial_data.sql and
--  events_table_corrected2.sql). They store master data about pets,
--  one-point advice messages, and raw events. The application can import
--  or reference these tables as needed without impacting the core schema.
...
CREATE TABLE IF NOT EXISTS roro_pet_master (
  ...
  population_rate  DECIMAL(5,2) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS roro_event_raw (
  ...
  KEY idx_roro_event_raw_date (date_text),
...
SQL;
  ...
  public static function dbdelta_tables($prefix, $collate) {
    return array(
       ...
       * External data tables
       *
       * These tables serve as import targets for pet master data, one‑point
       * advice messages and raw event listings. They are deliberately kept
       * separate from the core schema so that auxiliary datasets can be
       * updated independently.
       */
      "CREATE TABLE {$prefix}roro_pet_master (
        petm_id          VARCHAR(32) NOT NULL,
        pet_type         VARCHAR(16) NOT NULL,
        breed_name       VARCHAR(120) NOT NULL,
        category_code    VARCHAR(10) DEFAULT NULL,
      ...
      "CREATE TABLE {$prefix}roro_event_raw (
        event_id    VARCHAR(50) NOT NULL,
        name        VARCHAR(255) NOT NULL,
        date_text   VARCHAR(50) DEFAULT NULL,
        location    VARCHAR(255) DEFAULT NULL,
