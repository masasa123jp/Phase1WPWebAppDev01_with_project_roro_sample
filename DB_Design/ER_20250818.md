...
erDiagram
  ...
  RORO_AUTH_SESSION {
    ...
    BIGINT account_id FK         "→ RORO_AUTH_ACCOUNT.account_id"
    INT customer_id FK           "→ RORO_CUSTOMER.customer_id"
  ...
  RORO_BREED_MASTER {
    ...
    VARCHAR pet_type
    VARCHAR breed_name
    VARCHAR category_code FK     "→ RORO_CATEGORY_MASTER.category_code"
    INT population
    DECIMAL population_rate
    VARCHAR category_descprition
  ...
  RORO_TRAVEL_SPOT_MASTER {
    ...
    TEXT review
    VARCHAR category_code FK           "→ RORO_CATEGORY_MASTER.category_code（論理参照）"
    TIMESTAMP source_updated_at
    TIMESTAMP created_at
    TIMESTAMP updated_at
    BOOLEAN isVisible
  ...
  RORO_CATEGORY_DATA_LINK_MASTER {
    VARCHAR CDLM_ID PK         "例: CATEGORY_A_000001"
    ENUM   pet_type   PK           "DOG/CAT/OTHER"
    VARCHAR OPAM_ID FK              "→ RORO_ONE_POINT_ADVICE_MASTER.OPAM_ID"
    VARCHAR category_code FK        "→ RORO_CATEGORY_MASTER.category_code"
    VARCHAR GMAPM_ID FK             "→ RORO_GOOGLE_MAPS_MASTER.GMAPM_ID（お店マスタ）"
  ...
  RORO_RECOMMENDATION_LOG {
    ...
    VARCHAR CDLM_ID FK           "→ RORO_CATEGORY_DATA_LINK_MASTER.CDLM_ID（論理参照）"
    ENUM   pet_type     FK           "→ RORO_CATEGORY_DATA_LINK_MASTER.pet_type（冗長）"
    VARCHAR category_code FK         "→ RORO_CATEGORY_MASTER.category_code（冗長）"
    VARCHAR OPAM_ID     FK           "→ RORO_ONE_POINT_ADVICE_MASTER.OPAM_ID（冗長可）"
    VARCHAR GMAPM_ID    FK           "→ RORO_GOOGLE_MAPS_MASTER.GMAPM_ID（冗長可）"
    INT     rank                     "1位/2位…"
  ...
  RORO_AI_CONVERSATION ||--o{ RORO_AI_MESSAGE : "参照(conv_id)"


  RORO_ONE_POINT_ADVICE_MASTER ||--o{ RORO_CATEGORY_DATA_LINK_MASTER : "参照(OPAM_ID)"
  RORO_CATEGORY_MASTER ||--o{ RORO_CATEGORY_DATA_LINK_MASTER : "参照(category_code)"
  RORO_GOOGLE_MAPS_MASTER           ||--o{ RORO_CATEGORY_DATA_LINK_MASTER : "参照(GMAPM_ID)"
  ...
  RORO_CATEGORY_DATA_LINK_MASTER     ||..o{ RORO_RECOMMENDATION_LOG : "based_on(論理参照)"
