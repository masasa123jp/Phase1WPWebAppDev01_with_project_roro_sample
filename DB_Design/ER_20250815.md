...
erDiagram
  ...
  PET_MASTER {
    ...
    VARCHAR pet_type
    VARCHAR breed_name
    VARCHAR category_code "→ CATEGORY_MASTER.category_code"
    INT population
  ...
  RORO_TRAVEL_SPOT {
    ...
    VARCHAR category_code              "→ CATEGORY_DATA_LINK.CATEGORY_ID（論理参照）"
    BOOLEAN isVisible
  }
  ...
  RORO_CATEGORY_DATA_LINK {
    ...
    VARCHAR OPAM_ID                 "→ OPAM.OPAM_ID"
    VARCHAR category_code           "→ CATEGORY_MASTER.category_code"
    VARCHAR GMAPM_ID                "→ GMAPM.GMAPM_ID（お店マスタ）"
  ...
  RORO_RECOMMENDATION_LOG {
    BIGINT  rec_id PK
    INT     customer_id              "→ RORO_CUSTOMER"
    DATE    rec_date                 "推薦日(YYYY-MM-DD)"
    VARCHAR CATEGORY_ID              "→ CATEGORY_DATA_LINK.CATEGORY_ID（論理参照）"
    ENUM   pet_type                  "→ CATEGORY_DATA_LINK.pet_type（冗長）"
    VARCHAR category_code            "→ CATEGORY_MASTER.category_code（冗長）"
    VARCHAR OPAM_ID                  "→ OPAM.OPAM_ID（冗長可）"
    VARCHAR GMAPM_ID                 "→ GMAPM.GMAPM_ID（冗長可）"
    INT     rank                     "1位/2位…"
  ...
  RORO_CATEGORY_MASTER ||--o{ PET_MASTER : "category_code"

  RORO_CATEGORY_MASTER ||--o{ RORO_TRAVEL_SPOT : "参照(category_code)"
  ...
  RORO_OPAM            ||--o{ RORO_CATEGORY_DATA_LINK : "参照(OPAM_ID)"
  RORO_CATEGORY_MASTER ||--o{ RORO_CATEGORY_DATA_LINK : "参照(category_code)"
  RORO_GMAPM           ||--o{ RORO_CATEGORY_DATA_LINK : "参照(conv_id)"
  ...
  RORO_OPAM                   ||..o{ RORO_RECOMMENDATION_LOG : "冗長参照(任意)"
  RORO_GMAPM                  ||..o{ RORO_RECOMMENDATION_LOG : "冗長参照(任意)"
