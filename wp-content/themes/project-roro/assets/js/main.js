...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // WordPressではテンプレートファイルが .php 拡張子になるため、
  // ログイン不要ページのリストも .php に変更します。
  const unrestrictedPages = ['index.php', 'signup.php', ''];
  if (!unrestrictedPages.includes(currentFile)) {
    requireLogin();
  }
  // すでにログイン済みで index や signup ページを開いた場合はマップへリダイレクト
  if (isLoggedIn()) {
    if (currentFile === 'index.php' || currentFile === '' || currentFile === '/') {
