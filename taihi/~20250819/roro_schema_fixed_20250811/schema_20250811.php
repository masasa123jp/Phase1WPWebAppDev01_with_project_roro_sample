...
SQL;
  ...
  /** dbDelta table statements (Spatial/FK are applied later via ALTER). */
  ...
  /** Install via dbDelta → generated columns/Spatial → FKs */
  public static function install() {
    global $wpdb;

    // load dbDelta if needed
    ...
    if (!function_exists('dbDelta')) {
      // Not in WP context: give a clear guidance instead of fatal error
      throw new \RuntimeException('dbDelta() is not available. Run this inside WordPress (plugin activation or WP-CLI), or use raw SQL mode.');
    }
    ...
    foreach (self::dbdelta_tables($p, $collate) as $sql) {
      // Avoid static analysis error by using call_user_func
      call_user_func('dbDelta', $sql);
    }

    // Generated columns (MySQL 8+) + Spatial indexes
    $wpdb->query("ALTER TABLE {$p}roro_facility
      MODIFY facility_pt POINT SRID 4326
    ...
    $wpdb->query("ALTER TABLE {$p}roro_map_favorite
      MODIFY place_pt POINT SRID 4326
      GENERATED ALWAYS AS (IF(lat IS NULL OR lng IS NULL, NULL, ST_SRID(Point(lng,lat),4326))) STORED");
    ...
    // Foreign Keys (with existence check)
    ...
    self::maybe_add_fk("{$p}roro_user_link_wp", "fk_link_wp_user",
      "FOREIGN KEY (wp_user_id) REFERENCES {$p}users(ID) ON DELETE CASCADE ON UPDATE RESTRICT");

    self::maybe_add_fk("{$p}roro_auth_account", "fk_auth_customer",
      "FOREIGN KEY (customer_id) REFERENCES {$p}roro_customer(customer_id) ON DELETE CASCADE ON UPDATE RESTRICT");

    self::maybe_add_fk("{$p}roro_auth_session", "fk_sess_account",
      "FOREIGN KEY (account_id) REFERENCES {$p}roro_auth_account(account_id) ON DELETE CASCADE ON UPDATE RESTRICT");
    self::maybe_add_fk("{$p}roro_auth_session", "fk_sess_customer",
      "FOREIGN KEY (customer_id) REFERENCES {$p}roro_customer(customer_id) ON DELETE CASCADE ON UPDATE RESTRICT");

    self::maybe_add_fk("{$p}roro_auth_token", "fk_token_account",
      "FOREIGN KEY (account_id) REFERENCES {$p}roro_auth_account(account_id) ON DELETE CASCADE ON UPDATE RESTRICT");

    self::maybe_add_fk("{$p}roro_pet", "fk_pet_customer",
      "FOREIGN KEY (customer_id) REFERENCES {$p}roro_customer(customer_id) ON DELETE CASCADE ON UPDATE RESTRICT");
  ...
  public static function run_raw_sql() {
    ...
    foreach ($stmts as $stmt) {
      $s = trim($stmt);
      if ($s === '' or stripos($s, 'SET NAMES') === 0) { continue; }
      $wpdb->query($s);
    }
  }

  /** Add index if not exists */
  private static function maybe_add_index($table, $index, $ddl) {
    global $wpdb;
  ...
  private static function maybe_add_fk($table, $name, $fk_def) {
    global $wpdb;
    $exists = $wpdb->get_var($wpdb->prepare(
    ...
    if (!$exists) {
      $wpdb->query("ALTER TABLE {$table} ADD CONSTRAINT {$name} {$fk_def}");
    }
  }
}
}

// Optional: plugin activation hook (uncomment in your plugin main file)
