...
class Gacha_Endpoint extends Abstract_Endpoint {
     ...
     *
     * 親クラスの `permission_callback()` ではリクエストを受け取り
     *  nonce 検証やログインチェックを行います。PHP8 以降では子クラスの
     *  シグネチャが親と一致していないと致命的エラーになるため、ここでは
     *  WP_REST_Request パラメータを受け取り、ログインしているかどうか
     *  だけを判定して返します。
     *
    ...
    public static function permission_callback( WP_REST_Request $request ) {
        // ログインしていなければ false。nonce 検証は上位で実施される。
