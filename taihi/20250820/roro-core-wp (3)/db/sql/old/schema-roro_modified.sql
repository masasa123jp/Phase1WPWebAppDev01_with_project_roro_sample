...
-- PET_MASTER に対応する種別マスタ：犬・猫など種別別統計やカテゴリ情報を保持します。
...
-- 外部イベント取得データを格納するテーブル。イベントごとに緯度経度や開催地を保持します。
CREATE TABLE IF NOT EXISTS `roro_event_raw` (
  `event_id`    VARCHAR(50) NOT NULL COMMENT 'イベントID（外部ソースID）',
  `name`        VARCHAR(255) NOT NULL COMMENT 'イベント名',
  `date_text`   VARCHAR(50) DEFAULT NULL COMMENT '日付（文字列、YYYY-MM-DD あるいは範囲）',
