...
erDiagram
  ...
  RORO_CUSTOMER["顧客 / RORO_CUSTOMER"] {
    ...
    VARCHAR country_code "国コード(ISO-3166-1 alpha-2) / Country Code"
    VARCHAR prefecture "都道府県 / Prefecture"
    VARCHAR city "市区町村 / City"
    VARCHAR address_line1 "住所1（番地まで） / Address Line 1"
    VARCHAR address_line2 "住所2（建物・部屋等） / Address Line 2"
    VARCHAR building "建物名（任意） / Building"
  ...
  %% ペット（顧客1:N、代表ペットは任意）
  %% =========================
  PET_MASTER["犬種マスタ / PET_MASTER"] {
    VARCHAR PETM_ID PK "犬種ID / Breed ID"
  ...
  RORO_PET["ペット / RORO_PET"] {
    ...
    VARCHAR PETM_ID FK "犬種ID(→PET_MASTER.PETM_ID)"
    VARCHAR breed_label "犬種表示名(任意)"
  ...
  %%  緯度経度は保持。ただし RORO_FACILITY に確実に紐付く場合は
  %%  ソース側lat/lngは省略可（重複回避）。
  %% ======================================================
  GMAPM["施設マスタ(Google Mapsベース) / GMAPM"] {
    ...
    DECIMAL latitude "緯度(任意; 施設紐付け時は省略可)"
    DECIMAL longitude "経度(任意; 施設紐付け時は省略可)"
  ...
  RORO_FACILITY["施設(正規化・検索最適) / RORO_FACILITY"] {
    ...
    DECIMAL lat "緯度（正規化の主たる保持先）"
    DECIMAL lng "経度（正規化の主たる保持先）"
    POINT facility_pt "生成列: POINT(lng,lat) SRID 4326（SPATIAL INDEX対象）"
  }
  ...
  RORO_MAP_FAVORITE["地図お気に入り / RORO_MAP_FAVORITE"] {
    ...
    VARCHAR google_place_id "Google Place ID(任意)"
    VARCHAR label "ラベル"
    DECIMAL lat "緯度(任意: カスタム地点時)"
    DECIMAL lng "経度(任意: カスタム地点時)"
