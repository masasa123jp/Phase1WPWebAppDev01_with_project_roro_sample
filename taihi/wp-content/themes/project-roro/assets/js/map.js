 ...
 * Google Maps API ローダ
 *
 * この即時関数は、ページ読み込み時に Google Maps API を動的に読み込みます。
 * API キーはサーバー側から `wp_localize_script()` によって渡された `roro_globals.google_maps_api_key`
 * に格納されており、ハードコーディングされたキーを使用しません。既に `google` オブジェクトが
 * 存在する場合は単に `initMap()` を呼び出します。
 */
(function loadGoogleMaps() {
  if (typeof window === 'undefined') return;
  document.addEventListener('DOMContentLoaded', function () {
    const lang = typeof getUserLang === 'function' ? getUserLang() : 'ja';
    const key = window.roro_globals && window.roro_globals.google_maps_api_key;
    // 既に Google Maps ライブラリがロードされている場合は initMap を呼び出すだけ
...
function initHereMap() {
  ...
  // HERE Maps API キーはグローバル変数から取得します。公開されているコードにハードコーディングしないようにしています。
  const apikey = (window.roro_globals && window.roro_globals.here_maps_api_key) || '';
